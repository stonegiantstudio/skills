...
whatever branch happens to be checked out, which is how `/stone-giant:deep-review 123`
...
> - `/stone-giant:deep-review 123 --post` — also post the review to the PR
...
| `stone-giant:refactoring-legacy` | a feature was replaced or retired | Orphaned code, dead tests, half-finished expand/contract |
| `stone-giant:js-ninja` | any JS/TS | Type discipline, modern idioms, boundary defensiveness |
| `stone-giant:testing-ninja` | new logic, or a bug fix | Coverage for new branches; a fix carries a reproducing test |
| `stone-giant:react-router-v7` | React Router routes, loaders, actions | Route-module shape, loader/action discipline, param validation, URL-as-state |
| `stone-giant:design-ninja` | UI, visual, layout | Hierarchy, typography, tokens over raw values, hydration-safe rendering |
| `stone-giant:zod-ninja` | Zod / Conform / form schemas | Schema shape, cross-field validation |
| `stone-giant:better-auth` | auth, sessions, OAuth | Integration mechanics |
| `stone-giant:signup-signin` | sign-up / sign-in surfaces | Auth UX, copy, recovery affordances |
| `stone-giant:resend` | email send, templates | Send mechanics, template shape |
| `stone-giant:dates-and-times` | dates, timezones, clocks | Timezone correctness, clock injection |
| `stone-giant:technical-writing` | docs, README | Structure, accuracy, audience |
| `stone-giant:writing-markdown` | markdown structure; docs-only diffs | Formatting discipline |
| `stone-giant:ci-performance` | CI workflows, slow pipeline | Cache keys, job graph, wasted work |
| `stone-giant:npm-security-advisory` | package.json, lockfile, new deps | Advisories, compromised packages |
| `stone-giant:eval-npm` | a new package is introduced | Maintenance health, alternatives |
| `stone-giant:relational-db-theory` | schema / DDL changes | Normalization, keys, constraints, relational anti-patterns |
| `stone-giant:postgresql` | Postgres SQL or schema | Postgres types and features; JSONB vs column modeling |
| `stone-giant:drizzle-migrations` + `stone-giant:postgresql` | Drizzle schema, drizzle-kit, `migrations/` | Migration safety, schema drift |
| `stone-giant:kysely-orm` | Kysely query builder | Query builder idioms, type wiring |
| `stone-giant:sql-server` + `stone-giant:sql-server-safety` + `stone-giant:sql-server-performance` | T-SQL / Azure SQL | Types and naming; safety (guards, error handling, injection); SARGability and plan shape |
...
use; `stone-giant:relational-db-theory` applies to schema changes in any dialect.
...
- **Dependency bump only** — most lenses don't apply; say so and load
  `stone-giant:npm-security-advisory` and `stone-giant:eval-npm` instead.
- **Docs-only diff** — `stone-giant:technical-writing` and `stone-giant:writing-markdown`; skip the
